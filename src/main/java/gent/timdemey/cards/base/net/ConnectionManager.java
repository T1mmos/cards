...
public class ConnectionManager {
    ...
    public static void dropAllConnections() {
        established.values().stream().forEach(conn -> conn.stopImpl());
        established.clear();
    }

    public static void write(B_Message msg) {
        if (BROADCAST_ID.equals(msg.getCommand().getDestinationID())) {
            established.values().stream().filter(conn -> !BOOTING_ID.equals(conn.getId()))
                    .forEach(conn -> conn.write(msg));
        } else {
     ...
     * combination of given IP and port) and one can start writing to the
     * connection as well.
     * 
     * @param id
     */
